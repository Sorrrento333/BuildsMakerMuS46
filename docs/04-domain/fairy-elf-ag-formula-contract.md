# Contrato de AG de Fairy Elf

## Identidad y autoridad

- Claim: `DR-AG-FAIRY-ELF`, `VERIFIED`.
- Fórmula: `formula-ag-fairy-elf` `1.0.0`.
...
| `strength` | `INT64` | `CONTEXT_VALUE/resolved-strength` | mínimo factual 22, `EVD-0021` |

La expresión autorizada es:

```text
ag = energy * 0.2 + vitality * 0.3 + agility * 0.2 + strength * 0.3
```

La fórmula no consume nivel ni otra fórmula derivada. `CHECKED_DECIMAL_V1`
conserva `0.2` y `0.3` exactamente en base 10. El programa declara seis pasos
ordenados: cuatro aportes de stat, resultado crudo y resultado visible. La
aritmética decimal es comprobada, no hay redondeos intermedios y `visible-ag`
trunca hacia cero una sola vez.

## Casos aprobados

| Caso | Evolución | ENE/VIT/AGI/STR | AG crudo | AG visible |
|---|---|---|---:|---:|
| `ag-fairy-elf-base` | Fairy Elf | `15/20/25/22` | 20.6 | 20 |
| `ag-fairy-elf-energy-vitality-step` | Muse Elf | `16/21/25/22` | 21.1 | 21 |
| `ag-fairy-elf-agility-strength-step` | Muse Elf | `15/20/26/23` | 21.1 | 21 |
| `ag-fairy-elf-combined-step` | High Elf | `16/21/26/23` | 21.6 | 21 |
